...
class PlayState extends MusicBeatState
{
	...
	public function doScoreBop():Void {
		...
		scoreTxtTween = FlxTween.tween(scoreTxt, {y: scoreTxtBaseY}, 0.2, {onComplete: (f)->scoreTxtTween = null,ease: FlxEase.circOut});
	}

	public function runEventsBefore(time:Float)
	{
			for (note in eventNotes)
			{
				if (note.strumTime < time)
				{
						triggerEvent(note.event, note.value1, note.value2, note.strumTime);
				}
			}
	}
	...
	public function setSongTime(time:Float)
	{
		...
		runEventsBefore(time);

		var prevTime = Conductor.songPosition;
		...
		@:privateAccess
		for (tween in FlxTween.globalManager._tweens) //i think doing it to every tween would be preffered?
		{

			tween._secondsSinceStart += (time - prevTime) / 1000;
		}
	}
	...
	override public function update(elapsed:Float)
	{
		...
		//temp but quick and easy
		if (SONG.song.toLowerCase() == 'expurgation')
		{
			if (camZooming)
				FlxG.camera.zoom = FlxMath.lerp(defaultCamZoom * cameraZoomMult, FlxG.camera.zoom, Math.exp(-elapsed * 3.125 * camZoomingDecay * playbackRate));
		...
		final curScore = Std.parseInt(scoreTxt.text);
		if (!Math.isNaN(curScore))
		{
			var newScore = Math.floor(FlxMath.lerp(curScore, songScore, 0.3 * 60 * elapsed));
			if (Math.abs(newScore - curScore) <= 10) newScore = songScore;
		...
		//why not make this charting mode
		if (chartingMode #if debug || true #end)
		{
			...
			if (FlxG.keys.justPressed.SIX) cpuControlled = botplayTxt.visible = !botplayTxt.visible;
		}
	...
	function set_health(value:Float):Float // You can alter how icon animations work here
	{
		...
		playerBar.setCount(playerBar.remapToCount(2-health,0,2));
		oppBar.setCount(oppBar.remapToCount(health,0,2));
		...
		playerBar.changeFrameAnim(value >= 1.5 ? 'scare' : 'i');
		oppBar.changeFrameAnim(value <= 0.5 ? 'scare' : 'i');
	...
	function openPauseMenu()
	{
		...
		openSubState(new substates.MadnessPause());
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		switch(eventName) 
		{
			case 'forceFocusChange':
				switch (value1.toLowerCase())
				{
					case 'dad' | 'opponent': charFocus = dad;
					case 'bf' | 'boyfriend': charFocus = boyfriend;
					case 'gf' | 'girlfriend': charFocus = gf;
					case _: charFocus = null;
				}
			case 'zoomTween':
	...
	function moveCameraSection(?sec:Null<Int>):Void {
		...
		if (gf != null && SONG.notes[sec].gfSection)
		{
			...
			var displacement = returnDisplacePoint(gf);
		...
		final isDad:Bool = (SONG.notes[sec].mustHitSection != true);
		moveCamera(isDad);
	...
	inline public function returnDisplacePoint(char:Character):FlxPoint
	{
		var displace:FlxPoint = FlxPoint.weak();
		return switch (char.getAnimationName().substr(4).split('-')[0].toLowerCase()) {
			case 'up' | 'up-alt': displace.set(0,-camDisplacement);
			case 'down' | 'down-alt': displace.set(0,camDisplacement);
			case 'left' | 'left-alt': displace.set(-camDisplacement,0);
			case 'right' | 'right-alt': displace.set(camDisplacement,0);
			default: displace;
		}
	}
	...
	public function moveCamera(isDad:Bool)
	{
		...
		var curChar:Character = null;

		curChar = isDad ? dad : boyfriend;
		if (charFocus != null) curChar = charFocus;

		final desiredPos = getCharacterCameraPos(curChar);

		final displacement = returnDisplacePoint(curChar);
	...
	public function endSong()
	{
		...
		{
			//super fuckign temp
			if (SONG.song.toLowerCase() == 'expurgation') FlxG.save.data.beatExpurgation = true;
			if (SONG.song.toLowerCase() == 'assassination') FlxG.save.data.beatAssasin = true;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		rating.x += ClientPrefs.data.comboOffset[0] + FlxG.random.int(-10,10);
		rating.y -= ClientPrefs.data.comboOffset[1] - FlxG.random.int(-10,10);
		rating.antialiasing = antialias;


		final ratingXPoint:Float = rating.x;
		rating.x = ratingXPoint - (rating.width/2);
	...
	{
		if (gf != null && beat % Math.round(gfSpeed * gf.danceEveryNumBeats) == 0 && !gf.getAnimationName().startsWith('sing') && !gf.stunned && !gf.blockIdle)
			gf.dance();
		if (boyfriend != null && beat % boyfriend.danceEveryNumBeats == 0 && !boyfriend.getAnimationName().startsWith('sing') && !boyfriend.stunned && !boyfriend.blockIdle)
			boyfriend.dance();
		if (dad != null && beat % dad.danceEveryNumBeats == 0 && !dad.getAnimationName().startsWith('sing') && !dad.stunned && !dad.blockIdle)
			dad.dance();
	}
	...
	function strumPlayAnim(isDad:Bool, id:Int, time:Float) {
		if (ClientPrefs.data.trickyExperience) return;
